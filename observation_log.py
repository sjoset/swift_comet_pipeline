...
def build_observation_log(
    ...
    # print("Building list of images from FITS extensions ...")
    ...
    obs_log["PX"] = x_list
    obs_log["PY"] = y_list
    obs_log[["PX", "PY"]] = obs_log[["PX", "PY"]].astype(float)

    # Convert time columns back to str for writing
...
def read_observation_log(obs_log_path: pathlib.Path) -> SwiftObservationLog:
    ...
    """
    obs_log = pd.read_parquet(obs_log_path)
