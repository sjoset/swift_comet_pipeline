...
from swift_comet_pipeline.data_ingestion.observation_log.get_comet_position import (
...
from swift_comet_pipeline.stacking.event_mode_offsets import get_event_mode_pixel_offset
...
def _coincidence_correct_one_image(
    img_exp_and_scale: tuple[SwiftUvotImage, float, UvotPixelResolution]
) -> SwiftUvotImage:
...
def coincidence_correction_step(
    ...
    img_exp_and_scale = zip(
        ...
        [UvotPixelResolution.event_mode] * num_images,
    )
...
def event_mode_fits_to_time_binned_image(
    ...
    comet_ras_decs = [
        get_comet_position_at_time(horizons_id=precursor_img.horizons_id, mt=t)
    ...
    # Apply an offset to the images
    event_mode_pixel_offset = get_event_mode_pixel_offset()
    ...
    slice_comet_centers_pix = [
        PixelCoord(
            x=int(np.round(x - 1 + event_mode_pixel_offset.x)),
            y=int(np.round(y - 1 + event_mode_pixel_offset.y)),
        )
    ...
    #             scale=UvotPixelResolution.event_mode,
    #         )
    ...
    sum_result = StackableUvotImage(
        ...
        data_mode=UvotImageMode.event_mode,
    )
