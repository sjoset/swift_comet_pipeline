...
# TODO: add optional 'derived_from' entry to carry data structure that produced this profile
# to CometRadialProfile and CometProfile
...
class CometRadialProfile:
    ...
    profile_axis_rs: np.ndarray
    # the actual pixel values (count rates)
    pixel_values: np.ndarray


@dataclass
class CometRadialProfileFromConicalRegion(CometRadialProfile):
    """
...
class CometProfile:
    ...
    # as in CometRadialProfileFromConicalRegion
    profile_axis_rs: np.ndarray
    pixel_values: np.ndarray
    ...
    @classmethod
    def from_radial_profile(cls, radial_profile: CometRadialProfileFromConicalRegion):
        ...
        profile_axis_rs = -np.array(list(reversed(radial_profile.profile_axis_rs[1:])))

        return CometProfile(
            profile_axis_rs=np.concatenate(
                (profile_axis_rs, radial_profile.profile_axis_rs), axis=None
            ),
...
def comet_radial_profile_from_json(json_dict: dict) -> CometRadialProfile:
    return from_unstructured(obj=json_dict, c=CometRadialProfile)


def json_from_comet_radial_profile(crp: CometRadialProfile) -> dict:
    return to_unstructured(crp)


def comet_radial_profile_from_cone_from_json(
    json_dict: dict,
) -> CometRadialProfileFromConicalRegion:
    return from_unstructured(obj=json_dict, c=CometRadialProfileFromConicalRegion)


def json_from_comet_radial_profile_from_cone(
    crpfc: CometRadialProfileFromConicalRegion,
) -> dict:
    return to_unstructured(crpfc)
