...
from swift_comet_pipeline.scp_types.compound.plateau_list import (
    ReddeningToProductionPlateauListDict,
)
from swift_comet_pipeline.scp_types.primitive import *


# TODO: update this to use AnnularApertureProfile
def aperture_production_rate_analysis(
    eid: EpochIndexEntry,
    ...
    uw1_magnitude = magnitude_from_count_rate(
        count_rate=uw1_count_rate, filter_type=UvotFilter.uw1
    )
    uvv_magnitude = magnitude_from_count_rate(
        count_rate=uvv_count_rate, filter_type=UvotFilter.uvv
    )
    ...
    q_h2o = num_OH_within_r_to_Q_vectorial(
        helio_r_au=eid.rh_au,
        num_OH=num_oh,
        within_r=aperture_radius_pix * eid.km_per_pix * u.km,  # type: ignore
    )

    return QvsApertureRadiusEntry(
        aperture_r_pix=aperture_radius_pix,
        aperture_r_km=aperture_radius_pix * eid.km_per_pix,
...
#
#     r_pix_max = float(max_aperture_radius.to_value(u.km)) / epoch_summary.km_per_pix
#     if r_pix_max < 1.0:
#         print(
...
#         num_concentric_apertures=num_apertures,
#     )
...
#             ap=ap,
#             background=uw1_bg,
#             exposure_time_s=eid.exposure_times[UvotFilter.uw1],
#         )
...
#             ap=ap,
#             background=uvv_bg,
#             exposure_time_s=eid.exposure_times[UvotFilter.uvv],
#         )
...
#     ap_analysis_list = [
#         aperture_production_rate_analysis(
#             eid=eid,
