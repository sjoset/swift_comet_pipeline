...
class EpochImagePlot(object):
    ...
    def set_current_image_index(self, image_index: int) -> None:
        ...
        if image_index < 0 or image_index > self.current_image_count - 1:
            # print(f"Not updating image index to {image_index}")
            return
...
# class EpochImagePlotOld(object):
#     def __init__(self, scp: Products):
#         self.swift_data = SwiftData(data_path=scp.cfg.swift_data_path)
#
#         self.obs_log = scp.load_epoch_log()
#         assert self.obs_log is not None
#         self.epoch_ids = self.obs_log.epoch_id.unique()
#
#         # TODO: hash this and the epoch_log to look for changes, write if changed
#         self.original_epoch_log = self.obs_log.copy()
#
#         self.zscale = ZScaleInterval()
#
...
#         self.setup_plot_elements()
#
#         self.set_current_epoch(self.epoch_ids[0])
...
#         # self.mark_sources()
#
#     def set_current_epoch(self, epoch_id: EpochID):
#         self.epoch_id = epoch_id
#         assert self.obs_log is not None
#         self.current_epoch_log = self.obs_log[self.obs_log.epoch_id == self.epoch_id]
#         self.obs_log = self.obs_log.sort_values("FILTER").reset_index(drop=True)
#         self.num_images = len(self.obs_log)
#         self.epoch_title = self.epoch_id
#
#     def setup_plot_elements(self):
#         self.fig, self.ax = plt.subplots(1, 1, figsize=(10, 10))
#         self.fig.subplots_adjust(bottom=0.18)
...
#         self.img_slider_ax = self.fig.add_axes([0.1, 0.025, 0.55, 0.04])  # type: ignore
#         self.img_slider = EpochImageSlider(
#             self.img_slider_ax, self.num_images, activecolor="orange"
#         )
#         self.img_slider.on_changed(self.slider_update)
#
#         self.veto_ax = plt.axes([0.7, 0.025, 0.1, 0.04])  # type: ignore
#         self.approve_ax = plt.axes([0.85, 0.025, 0.1, 0.04])  # type: ignore
#
...
#             centered_on=PixelCoord(x=x, y=y)
#         )
#         self.obs_log.at[self.current_image_index, "USER_CENTER_X"] = (
#             self.selected_comet_coords.x
#         )
#         self.obs_log.at[self.current_image_index, "USER_CENTER_Y"] = (
...
#         )
#
#         if self.current_epoch_row.FILTER == UvotFilter.uw1:
#             method = self.comet_center_finding_method_uw1
#         elif self.current_epoch_row.FILTER == UvotFilter.uvv:
#             method = self.comet_center_finding_method_uvv
#         else:
#             method = CometCenterFindingMethod.pixel_center
#
...
#
#         self.current_image_index = image_index
#         self.current_epoch_row = self.obs_log.iloc[self.current_image_index]
#
#         # self.current_image_path = (
#         #     self.swift_data.get_observation_image_directory(
...
#         self.current_image_path = self.current_epoch_row.FULL_FITS_PATH
#         self.current_image = self.swift_data.get_observation_image(
#             obsid=self.current_epoch_row.OBS_ID,
#             image_mode=self.current_epoch_row.DATAMODE,
...
#
#         # adjust scale for event mode images
#         if self.current_epoch_row.DATAMODE == UvotImageMode.event_mode:
#             self.current_image = np.log(self.current_image + 1)
#
#         self.vmin, self.vmax = self.zscale.get_limits(self.current_image)
...
#         assert self.obs_log is not None
#         epoch_row = self.obs_log.iloc[self.current_image_index]
#         # this assumes that the event mode images are not downsampled
...
#     def veto_current_image(self, _):
#         assert self.obs_log is not None
#         self.obs_log.loc[self.current_image_index, "manual_veto"] = np.True_
#         self.update_plot()
#
#     def approve_current_image(self, _):
#         assert self.obs_log is not None
#         self.obs_log.loc[self.current_image_index, "manual_veto"] = np.False_
#         self.update_plot()
#
#     def colormap_by_veto(self):
#         assert self.obs_log is not None
#         if self.obs_log.loc[self.current_image_index, "manual_veto"] is np.True_:
#             cmap = "binary"
#         else:
#             cmap = "magma"
#         return cmap
...
#         # if epoch_row.FILTER == UvotFilter.uvv:
#         #     self.mark_sources_uvv()
#         # else:
#         #     self.mark_sources_uw1()
#
