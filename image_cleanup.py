...
# ) -> SwiftUVOTImage:
#     aper = math.ceil(5.0 / float(detector_scale.value))
#     area_frac = np.pi / 4
#
#     i_len = len(image)
#     j_len = len(image[0])
...
#             # sjo
#             vertex += image[
#                 (2 * aper - 1 - i) : (i_len - i), (2 * aper - 1 - j) : (j_len - j)
#             ]
#
...
#             # sjo
#             coi_map_part += vertex[
#                 (1 - i) : (i_len - 2 * aper + 1 - i),
#                 (1 - j) : (j_len - 2 * aper + 1 - j),
#             ]
#
...
def coincidence_correction_orig(img_data: SwiftUVOTImage, scale: SwiftPixelResolution):
    # import pandas as pd
    aper = math.ceil(5.0 / float(scale))
    area_frac = np.pi / 4
    # i_ind = np.arange(0,len(img_data))
